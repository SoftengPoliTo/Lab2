...
|  Nominal Scenario     | User enters the email, the username, and the password and sends the sign-in request. The application validates and confirms |
...
|  Precondition     | |  
|  Post condition     |  |
|  Nominal Scenario     | User selects the type of fuel and enters the maximum distance from his location. Application uses Google Maps API to retrieve the user location and displays on the map all the gas station according to the user's choices|
