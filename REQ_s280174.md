...
|  2     | Picks the usual Petrol Station from the Favourites |
...
| Step#        |  Step description   |
|  1     | User opens the App |  
|  2     | A compulsory update is available for the App  |
...
- The App could show the variation of Price in Favourites to the side of the actual absolute Price, just about the previous day;
- The App should provide a GUI to compare trends in the last month
- Every update to the app is compulsory before its usage;
- The App may send a notification to the user autonomously thanks to the application space;
...
|  Precondition     | No update from SC2 is necessary  |  
|  Post condition     |  Service delivered | 
|  Nominal Scenario     | User opens the App, search for a petrol station, add it to the Favourites, close the App |
...
|  Precondition     | No update from SC2 is necessary  |  
|  Post condition     |  Service delivered | 
|  Nominal Scenario     | User opens the App, selects a subset of petrol stations, compare Prices during the last month, close the App |
|  Variants     |  |
...
|  FR3     | Select the Favourites, query the DB for the Favourites and petrol stations from FR5 only if it wasn't done in Current session, show both actual and Previous Price |
...
|  NFR4     | Space | The App stores no Prices at opening time, just the Favourites | FR3, FR2 |
...
|  NFR8     | Usability | The App visualizes if petrol stations are open and send a warn if a closed petrol station is selected | FR1, FR3, FR7, FR8 |