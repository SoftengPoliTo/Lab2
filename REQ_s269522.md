...
| ----------------- |:-----------:|
| Car drivers     |Uses the application to search the best gas station| 
...
:Car driver: -- (EZGas)
:Gas Sations owner: -- (EZGas)
```

## Interfaces
| Actor | Logical Interface | Physical Interface  |
| ------------- |:-------------:| -----:|
|Car Driver|GUI |Screen, keyboard or Touch screen|
...
For every gas station that Bob visit, he can write a review visible by all the other car drivers but also by the gas stations owners.

Pablo is a Gas station attendant and he wants to promote his activity in order to achieve more customers, he can sign up in the EZGas application by writing all the informations in terms of services and prices, he can write also weekly (or daily) promotion.
...
| ID        | Description  |
| ------------- |:-------------:| 
|  FR1     | Record the gas station location |  
|  FR2     | Record the gas station price |
|  FR3     | Record the gas station weekly (or daily) promotions |
...
```plantuml
left to right direction
actor :Car driver: as a1
actor :Gas stations owner: as a2
a1 -- (FR4  Record the favourite gas stations)
a1 -- (FR5 Record the reviews for every gas station)
a2 -- (FR1 Record gas station location)
a2 -- (FR2 Record gas station prices)
a2 -- (FR3 Record gas station promotions)
```
## Use Cases
...
| ------------- |:-------------:| 
|  Precondition     | CarDriver exist and CarDriver.numberVisited > 0|  
|  Post condition     | CarDriver.numberFavourites >= 0 |
|  Nominal Scenario     | The Car driver updates its list of favourite gas stations|
|  Variants     |  |
...
| Actors Involved        | Car driver  |
| ------------- |:-------------:| 
|  Precondition     | CarDriver exist ,Gas station (GS) exists , CarDriver.numberVisited > 0|  
|  Post condition     |  GS.numberReviews > 0|
|  Nominal Scenario     | Car driver writes a review of one of the gas stations that he visited|
|  Variants     | |
...
| ------------- |:-------------| 
| Description | CarDriver adds one gas station to his favourite|
| Precondition | CarDriver exists|
| Postcondition |  count of favourites of Car Driver increments |
| Step#        |  Step description   |
|  1     | CarDriver visit Gas station GS |  
|  2     | number of CarDriver gas station visited increment |
|  3     | CarDriver update its favourite list |
| 4 | The number of CarDriver favourites increases|

## Scenario 2

| Scenario ID: SC2        | Corresponds to UC5  |
| ------------- |:-------------| 
| Description | CarDriver writes a review and a Gas station owner (GSO) read the rerview|
|Precondition |  CarDriver exist ,Gas station (GS) exists|
